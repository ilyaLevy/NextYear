...
def main():
    ...
    # fix the DBSeed, just in case
    dbseed = 'C:\\Dev\\Quick\\FirstYear\\db\\dbseed.xml'
    tree = ET.parse(dbseed)
    root = tree.getroot()

    for photo in root.findall('photo'):
        if( not photo.attrib['url'].startswith('http') ):
            print photo.attrib['id'], ' - url '
            photo.set('url',fyp.getSharedUrl(dbxRoot + photo.attrib['url'] + ".jpg"))
            tree.write(dbseed)
            time.sleep(1)

    localDb = 'C:\\Dev\\Quick\\FirstYear\\localDb.xml'
    tree = ET.parse(localDb)
    root = tree.getroot()
